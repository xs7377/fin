...
<div id="jump" hidden="">
	...
	<form action="./auction${kind}" id="wFrm" method="post">
		<div id="hd"></div>
		<input type="text" name="m_id" value="${auc.m_id }"><br>
		<input type="text" name="title" placeholder="title" value="${auc.title}"><br>
		<textarea rows="" cols="" id="contents" style="height: 200px;width: 100%;" name="contents">${auc.contents}</textarea><br>
		댓글허용여부<br>
		예<input type="radio" name="reply" value="y" class="isReply">
		아니오<input type="radio" name="reply" value="n" class="isReply"><br>
		프리미엄 물품등록<br>
		<div id="hid">
			<label for="priy">Yes</label>
			<label for="prin">No</label>
		</div>
		<input id="priy" type="radio" name="primeum" value="y" class="isPri">
		<input id="prin" type="radio" name="primeum" value="n" class="isPri"><br>
		<input type="number" name="min_price" placeholder="min_price" id="minP" value="${auc.min_price }"><br>
		<input type="number" name="max_price" placeholder="max_price" id="maxP" value="${auc.max_price }"><br>
		경매마감시간<br>
		<div id="datePicker"></div>
		<div id="temp"></div>
		<select id="period_time" name="period">
	...
	<div id="status1">
		<c:forEach items="${img}" var="i">
			<span><img alt="" src="../resources/upload/${i.fName}" width="100px;" height="100px;"><input type="button" id="${i.fName}" class="orgImg" value="삭제"></span>
		</c:forEach>
	</div>	
	
	<br>
	<input type="button" id="aucWBtn" value="등록하기">

	<input type="button" id="testBtn" value="test">
	
	<div id="jump" hidden="">
	<form action="./auctionList" method="post" name="jumpFrm">
